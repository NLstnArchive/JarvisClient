...
public class InputDispatcher {
	...
	public InputDispatcher() {
		...
		thread = new Thread("CommandDispatcherThread") {
			public void run() {
				while (JarvisMain.getInputModule().isRunning())
	...
	private void processCommand(String line) {
		...
		Runnable runnable = JarvisMain.getCommandModule().getCommandBuilder().buildCommand(parts[0], (Object[]) args);
		if (runnable == null) {
			Logger.error("Command '" + line + "' could not be compiled!", Level.LVL2);
			return;
		}
		JarvisMain.getInputModule().getThreadPool().submit(JTaskBuilder.newTask().executing(runnable));
	}
